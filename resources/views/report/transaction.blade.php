        ...
        <table class="table mt-4 align-middle">
            <thead>
                <tr>
                    ...
                    <th>Member</th>
                    <th>jumlah barang</th>
                    <th style="max-width: 200px">jaminan</th>
                    <th>status</th>
                    <th></th>
                </tr>
            </thead>
            <tbody id="transaction-list">
                ...
                @foreach ($transactions as $transaction)
                    <tr>
                        <td>
                            <p class="fw-bold mb-0" style="font-size:13px">{{ $transaction->id }}</p>
                        </td>
                        <td>
                            <div class="d-flex align-items-center">
                                <img src="{{ asset('storage/' . $transaction->member->photo) }}" alt="{{ $transaction->member->photo }}"
                                    style="width: 45px; height: 45px" class="rounded" />
                                <div class="ms-3">
                                    <p class="fw-bold mb-0" style="font-size:13px">{{ $transaction->member->name }}</p>
                                    <p class="text-muted mb-0" style="font-size: 11px">{{ $transaction->member->hp }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="p-2">
                            <div class="ms-3">
                                <p class="fw-bold mb-0" style="font-size:13px">{{ $transaction->total }}</p>
                                <p class="text-muted mb-0" style="font-size: 11px">{{ $transaction->transaction_date }}</p>
                            </div>
                        </td>
                        <td style="font-size:13px">{{ $transaction->collateral }}</td>
