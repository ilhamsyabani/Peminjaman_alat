                    ...
                    <div class="row mb-2">
                        <form action="{{ route('post.update', $post) }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="col-sm-8">
                                <!-- Form -->
                                <div class="mb-3">
                                    <img scr="{{ asset('storage/' . $post->gambar) }}" id="image-preview" alt="image preview" class="col-sm-6" />
                                    <br />
                                    <label for="gambar" class="form-label">Gambar Barang</label>
                                    <small id="gambar" class="form-text text-muted">Ukuran gambar maksimal 1MB dengan
                                        resolusi 800 x 1200 px</small>
                                    <input class="form-control @error('gambar') is-invalid @enderror" type="file" id="image-source" name="gambar" onchange="previewImage();" value="{{ old('gambar', $post->gambar) }}">
                                    @error('gambar')
                                ...
                                <div class="mb-2">
                                    ...
                                    <input type="text" class="form-control @error('title') is-invalid @enderror"
                                        id="title" name="title" value="{{ old('title', $post->title ) }}" aria-describedby="barangHelp">
                                ...
                                <div class="mb-2">
                                    ...
                                    <select class="form-select @error('category') is-invalid @enderror" id="category" name="category" aria-label="Default select example">
                                        ...
                                        <option value="about" {{ $post->category == 'about' ? 'selected' : '' }}>about</option>
                                    </select>
                                    @error('category')
                                        <div class="invalid-feedback"> {{ $message }}</div>
                                    @enderror
